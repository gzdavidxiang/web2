	...
	if(password.indexOf(" ") != -1){
		...
		if (password != confirm_password) {
		  ...
		  document.forms['formUser'].elements['Submit'].disabled = 'disabled';
			...
			document.getElementById('confirm_password_notice').style.color = "#093";
			$("#pwd_notice").hide();
			$("#pwd_intensity").show();
...
function check_confirm_password(confirm_password) {
	...
	if(confirm_password.indexOf(" ") != -1){
		document.getElementById('confirm_password_notice').innerHTML = "登录密码不能包含空格";
		document.getElementById('confirm_password_notice').style.color = "#900";
		$("#confirm_password_notice").show();
    $('#div_confirm_password').show();
    $('#item_confirm_password').css('margin-bottom', 0);
		document.forms['formUser'].elements['Submit'].disabled = 'disabled';
		return false;
    ...
    $('#item_confirm_password').css('margin-bottom', '15px');
		document.forms['formUser'].elements['Submit'].disabled = '';
		return false;
	}
	return true;
...
function checkUserName(inputObj) {
  var usernameObj = $(inputObj);
  var username = usernameObj.val();

  if (username == '') {
    $('#div_username_notice').show();
    $('#username_notice').html('用户名不能为空！');
    $('#username_notice').css({ color: '#900' });
    $('#item_username').css('margin-bottom', 0);
    //inputObj.focus();
		document.forms['formUser'].elements['Submit'].disabled = 'disabled';
	} else if (!/^[a-zA-Z\d]{6,20}$/.test(username)) {
    $('#div_username_notice').show();
		$('#username_notice').html('用户名长度为6到20位英文字母或数字！');
    $('#username_notice').css({ color: '#900' });
    $('#item_username').css('margin-bottom', 0);
    //inputObj.focus();
		document.forms['formUser'].elements['Submit'].disabled = 'disabled';
    ...
    $('#div_username_notice').show();
		$('#username_notice').html('用户名不能有中文！');
    $('#username_notice').css({ color: '#900' });
    $('#item_username').css('margin-bottom', 0);
    //inputObj.focus();
		document.forms['formUser'].elements['Submit'].disabled = 'disabled';
	} else {
		//Ajax.call('user?act=check_username', 'username=' + username, checkUserNameCallback, 'GET', 'TEXT', true, true);
    $.get('user', {
      act: 'check_username',
      username: username,
      ...
      if (data) {
        ...
        $('#div_username_notice').hide();
        $('#item_username').css('margin-bottom', '15px');
        document.forms['formUser'].elements['Submit'].disabled = '';
      } else {
        //document.getElementById('username_message').innerHTML = '用户名已存在，请重新输入！';
        $('#div_username_notice').show();
        $('#username_notice').html('用户名已存在，请重新输入！');
        $('#username_notice').css({ color: '#900' });
        $('#item_username').css('margin-bottom', 0);
        //inputObj.focus();
        document.forms['formUser'].elements['Submit'].disabled = 'disabled';
...
function checkFactName(inputObj) {
  var factnameObj = $(inputObj);
  var factname = factnameObj.val();

  if (factname == '') {
    $('#div_factname_notice').show();
    $('#factname_notice').html('姓名不能为空');
    $('#factname_notice').css({ color: '#900' });
    $('#item_factname').css('margin-bottom', 0);
    //inputObj.focus();
		document.forms['formUser'].elements['Submit'].disabled = 'disabled';
  } else if (!factname.match(/[\u4e00-\u9fa5]/)) {
    $('#div_factname_notice').show();
		$('#factname_notice').html('姓名不是中文！');
    $('#factname_notice').css({ color: '#900' });
    $('#item_factname').css('margin-bottom', 0);
    //inputObj.focus();
		document.forms['formUser'].elements['Submit'].disabled = 'disabled';
	} else if (factname.length < 2 || factname.length > 4) {
    $('#div_factname_notice').show();
		$('#factname_notice').html('姓名长度为2到4个中文字！');
    $('#factname_notice').css({ color: '#900' });
    $('#item_factname').css('margin-bottom', 0);
    //inputObj.focus();
		document.forms['formUser'].elements['Submit'].disabled = 'disabled';
  } else {
    $('#div_factname_notice').hide();
    $('#item_factname').css('margin-bottom', '15px');
    document.forms['formUser'].elements['Submit'].disabled = '';
    return;
		//Ajax.call('user?act=check_factname', 'factname=' + factname, checkFactNameCallback, 'GET', 'TEXT', true, true);
    $.get('user', {
      act: 'check_factname',
      factname: factname,
    }, function (data, textStatus, jqXHR) {
      if (data) {
        ...
        $('#div_factname_notice').show();
        $('#factname_notice').html('姓名已存在，请重新输入！');
        $('#factname_notice').css({ color: '#900' });
        $('#item_factname').css('margin-bottom', 0);
        document.forms['formUser'].elements['Submit'].disabled = 'disabled';
      }
...
function checkFactNameCallback(result) {
	if (result == 'true') {
    $('#div_factname_notice').hide();
    $('#item_factname').css('margin-bottom', '15px');
		document.forms['formUser'].elements['Submit'].disabled = '';
...
function checkIdCard(inputObj) {
  var idcardObj = $(inputObj);
  var idcard = idcardObj.val();

  if (idcard == '') {
    $('#div_idcard_notice').show();
    $('#idcard_notice').html('身份证不能为空');
    $('#idcard_notice').css({ color: '#900' });
    $('#item_idcard').css('margin-bottom', 0);
    //inputObj.focus();
		document.forms['formUser'].elements['Submit'].disabled = 'disabled';
  } else {
		//Ajax.call('user?act=check_idcard', 'idcard=' + idcard, checkIdCardCallback, 'GET', 'TEXT', true, true);
    $.get('user', {
      act: 'check_idcard',
      idcard: idcard,
    }, function (data, textStatus, jqXHR) {
      if (data == '0022') {
        $('#div_idcard_notice').show();
        $('#idcard_notice').html('身份证不合法，请重新输入！');
        $('#idcard_notice').css({ color: '#900' });
        $('#item_idcard').css('margin-bottom', 0);
        document.forms['formUser'].elements['Submit'].disabled = 'disabled';
      } else if (data == '0023') {
        $('#div_idcard_notice').show();
        $('#idcard_notice').html('身份证已注册！');
        $('#idcard_notice').css({ color: '#900' });
        $('#item_idcard').css('margin-bottom', 0);
        document.forms['formUser'].elements['Submit'].disabled = 'disabled';
      } else {
        $('#div_idcard_notice').hide();
        $('#item_idcard').css('margin-bottom', '15px');
        document.forms['formUser'].elements['Submit'].disabled = '';
      }
    });
	}
...
function checkIdCardCallback(result) {
	if (result == '0022') {
    $('#div_idcard_notice').show();
    $('#idcard_notice').html('身份证不合法，请重新输入！');
    $('#idcard_notice').css({ color: '#900' });
...
function checkMobilePhone(mobile, callback) {
	...
	if (mobile == '') {
    $('#div_mobile_phone_notice').show();
    $('#item_mobile_phone').css('margin-bottom', 0);
...
function checkMobilePhoneExist(mobile, callback) {
	...
	$.post('register?act=check_mobile_exist', {
		mobile_phone: mobile
	}, function(result) {
		if (result == 'false') {
      $('#div_mobile_phone_notice').hide();
      $('#item_mobile_phone').css('margin-bottom', '15px');
...
function sendMobileCode(mobileObj, mobileCodeObj, sendButton) {
	checkMobilePhone(mobileObj, function(result) {
		if (result) {
			// 发送邮件
			var url = 'register?act=send_mobile_code';
			$.post(url, {
				//XDEBUG_SESSION_START: 'ECLIPSE_DBGP',
