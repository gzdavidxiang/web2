...
function changeDelivery(obj, idx) {
  var old_dm = document.getElementById('order_' + idx + '_dm');
	if (old_dm.value == obj.value) {
		return;
	}
  /*
  var need_addr_obj = document.getElementById('need_addr');
  if (obj.value == 1 && need_addr_obj.value == 'true') {
    return alert('请选择地址！');
  }
  */

	Ajax.call('flow?step=change_delivery', 'delivery=' + obj.value + '&idx=' + idx, changeDeliveryResponse, 'GET');
}

function changeDeliveryResponse() {
  location.reload(true);
}

function check_before_submit2() {
  return location.href = 'flow?step=pay'; 
	//Ajax.call('flow?step=to_pay', '', checkOrderResponse, 'GET', 'JSON');
}

function checkOrder(obj, idx) {
  //return location.href='flow?step=go_pay&idx=' + idx;
	Ajax.call('flow?step=go_pay', 'idx=' + idx, checkOrderResponse, 'GET', 'JSON');
}
...
function zhixiashi(province) {
  var provinceId = province.value;
  return (provinceId != 1 && provinceId != 2 && provinceId != 31 && provinceId != 44);
}
function checkConsignee2(frm) {
	var msg = [];
	var err = false;

  var province = frm.elements['province'];
	if (province && province.value == 0 && province.length > 1) {
		err = true;
		msg.push(province_not_null);
	}

  var city = frm.elements['city'];
  //console.log('city:', city.value, !zhixiashi(province), city.length);
	if (city && city.value == 0 && !zhixiashi(province) && city.length > 1) {
    console.log('city oops');
    return false;
		err = true;
		msg.push(city_not_null);
	}
