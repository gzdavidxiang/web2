...
function check_factname(val) {
	if (val == '') {
		document.getElementById('factname_message').innerHTML = '姓名不能为空！';
	} else if (!val.match(/[\u4e00-\u9fa5]/)) {
		document.getElementById('factname_message').innerHTML = '姓名不是中文！';
	} else if (val.length < 2 || val.length > 4) {
		document.getElementById('factname_message').innerHTML = '姓名长度为2到4个中文字！';
  }
}

function check_idcard(val) {
  if (val == '') {
		document.getElementById('idcard_message').innerHTML = '身份证不能为空！';
  } else {
    $.get('user', {
      act: 'check_idcard',
      idcard: val,
      ...
      if (data == '0022') {
		    document.getElementById('idcard_message').innerHTML = '身份证不合法，请重新输入！';
      } else if (data == '0023') {
		    document.getElementById('idcard_message').innerHTML = '身份证已注册！';
      }
    });
  }
}
